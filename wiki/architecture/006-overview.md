...
5. **검증 범위 한정 (FULL-only) + Allowlist 로그한정 + html 단방향**: `check:architecture` drift는 FULL(`npm run verify`/pre-push)에서만 포착되며 QUICK(`verify:quick`=lint+vault+test)에서는 미포착이 정상이다. Allowlist 경로가 실제 파일로 존재하지 않으면 `[MISSING_COMPONENT]` 로그만 남기고 exit 0 — reviewer advisory로 정리 대상을 지정할 뿐 게이트를 차단하지 않는다. `architecture.html` 수동편집 금지 — md 수정 후 `npm run sync:architecture` 단방향 재생성만 허용 (코드 무수정, `sync-architecture.js` 불변).
